...
class Index extends Component
{
    ...
    {
        $question = QuestionBankQuestion::withTrashed()->find($id);
        if ($question->trashed()) {
            $question->forceDelete();
            session()->flash('message', 'Question permanently deleted.');
        } else {
            $question->delete();
            session()->flash('message', 'Question moved to trash.');
        }
    }

    public function restore($id)
    {
        QuestionBankQuestion::withTrashed()->find($id)->restore();
        session()->flash('message', 'Question restored.');
    }
